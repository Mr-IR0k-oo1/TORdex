...
use s3::{Auth, Client as S3Client, Credentials, Region};
...
async fn main() -> Result<()> {
    ...
    let _minio_client = S3Client::builder(&config.minio.endpoint)
        .context("building MinIO client")?
        .region(Region::new(&config.minio.region).context("invalid MinIO region")?.as_str())
        .auth(Auth::Static(
            Credentials::new(
                config.minio.access_key.clone(),
                config.minio.secret_key.clone(),
            )
            .context("invalid MinIO credentials")?,
        ))
        .build()
        .context("building MinIO client")?;
...
async fn readyz(Extension(health): Extension<Arc<HealthState>>) -> impl IntoResponse {
    let pg = health.postgres_ok.load(std::sync::atomic::Ordering::Relaxed);
    let rd = health.redis_ok.load(std::sync::atomic::Ordering::Relaxed);
    if pg && rd {
        (
            StatusCode::OK,
