//! Routing logic that picks a collector for each source.
...
/// Rate limiter keyed by host. In-memory only for the first build.
type HostLimiter = RateLimiter<NotKeyed, InMemoryState, DefaultClock>;

/// The router: owns the set of collectors, the rate limiter, the event bus,
/// and the collection store. Cheap to clone (`Arc` inside).
#[derive(Clone)]
...
impl CollectionRouter {
    /// Build a new router with the given HTTP collector, event bus, and store.
    /// Browser collectors are registered later via [`Self::with_collector`].
    pub fn new(
    ...
    /// Register an additional collector (e.g., a browser backend).
    #[must_use]
    ...
    /// Run a collection for `source`. Records the attempt in the store and
    /// emits a completion/failure event.
    pub async fn run(
        &self,
        ...
        // Persist a "running" row so the API can poll for status.
        self.store
        ...
        // Auto-escalation: if HTTP returned an empty/JS-heavy page, try a
        // browser backend once. We never escalate browser → anything else.
    ...
    async fn dispatch(&self, ctx: &CollectionContext) -> Result<CollectionResult, CollectionError> {
        let kind = self.pick_collector_kind(&ctx.source);
        let collector = self
            .collectors
            .get(&kind)
            .cloned()
            .ok_or_else(|| CollectionError::BrowserUnavailable(format!("{kind:?} not registered")))?;
    ...
    async fn emit_completed(&self, result: &CollectionResult) {
        ...
        let envelope = EventEnvelope::new("collection.completed", event);
    ...
    async fn emit_failed(&self, result: &CollectionResult, error: &str) {
        ...
        let envelope = EventEnvelope::new("collection.failed", event);
        if let Err(e) = self.events.publish(&envelope).await {
