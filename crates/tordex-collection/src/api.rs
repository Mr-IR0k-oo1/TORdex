//! HTTP API for the collection router.
...
/// Shared state for the collections router.
#[derive(Clone)]
...
/// Routes mounted under `/collections`.
...
async fn create_collection(
    ...
    // Idempotency-Key honoured here. If a row already exists for this
    // (source_id, idempotency_key), return it instead of starting a new run.
    ...
    // Spawn the actual collection. We return 202 Accepted immediately so the
    // client can poll.
...
/// API error type. Mirrors the one in `tordex-sources::api` but is a separate
/// type so each crate owns its error mapping.
#[derive(Debug)]
pub enum ApiError {
    NotFound(String),
    Conflict(String),
