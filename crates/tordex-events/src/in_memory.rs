...
//!
//! Uses `tokio::sync::broadcast` so that every subscriber sees every event.
//! Bounded buffer prevents runaway memory in tests if no subscriber drains.
...
/// In-memory event bus. Cheap to clone and share across tasks.
#[derive(Clone)]
...
impl EventBus for InMemoryEventBus {
        ...
        T: DeserializeOwned + Send + 'static,
    {
        ...
        let stream = async_stream::stream! {
            loop {
                match rx.recv().await {
                    Ok(bytes) => match EventEnvelope::<T>::decode(&bytes) {
