...
impl AppConfig {
    pub fn from_env() -> Result<Self, CoreError> {
        ...
        if redis_url.is_empty() {
            return Err(CoreError::Config("missing REDIS_URL".into()));
        }
        ...
        let http = HttpCollectorConfig {
            ...
            max_redirects: u8::try_from(parse_env::<u64>(
                "TORDEX_HTTP_MAX_REDIRECTS",
                u64::from(DEFAULT_HTTP_MAX_REDIRECTS),
            )?)
            .map_err(|_| CoreError::Config("invalid TORDEX_HTTP_MAX_REDIRECTS: out of range".into()))?,
        };
